...
import { Search, ArrowUpDown, Filter, ChevronLeft, ChevronRight, MoreHorizontal } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <div className="relative max-w-md w-full">
          ...
          <input
            type="text"
            placeholder="Search intakes by product, vendor, or ID..."
            value={searchTerm}
            onChange={(e) => {
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50/50 text-slate-500 text-xs border-b border-slate-200/60">
              <tr>
                ...
                <Th label="Product" sortKey="product" />
            ...
            <tbody className="divide-y divide-slate-100">
              {currentData.length === 0 ? (
                <tr>
                  <td colSpan="10" className="px-6 py-16 text-center text-slate-500 bg-white">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-sm mt-1 max-w-sm text-center">We couldn't find any requests matching your criteria. Try adjusting your search filters.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                currentData.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group border-b border-slate-50 last:border-none">
                    ...
                    <td className="px-5 py-3.5 text-slate-900 font-medium">{item.product}</td>
                    <td className="px-5 py-3.5 text-slate-600">{item.vendor}</td>
                    <td className="px-5 py-3.5 text-slate-600 text-right">{item.quantity}</td>
                    ...
                    <td className="px-5 py-3.5">
                      <span
                        className={cn(
                          "inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold tracking-wider uppercase border",
